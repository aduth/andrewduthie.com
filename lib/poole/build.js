    ...
    for (var sitePath in global.site) {
        ...
        fs.mkdir(dir, function() {
            ...
            site.forEach(function(item) {
                var outputPath = path.normalize(global.config.output + '/'  + sitePath + '/' + item.name + '.html'),
                    templatePath;
                ...
                if (item.meta.template) {
                    ...
                    renderTemplate(templatePath, outputPath, item);
                } else {
                    fs.writeFile(outputPath, item.content, 'utf8');
                }
            });
        });
    }
